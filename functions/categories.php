...
function getCategoriesByProject($idProject) {
    $cnx = connection();
    $rqt = $cnx->prepare('SELECT nomCat, idCat FROM categorie JOIN projet_cat ON projet_cat.refCat = categorie.idCat AND projet_cat.refProjet = ?');
...
function addCategorieToProject($idProject, $idCat) {
    ...
    $rqt->execute(array($idProject, $idCat));
...
function getCategoriesByMember($idMember) {
    $cnx = connection();
    $rqt = $cnx->prepare('SELECT nomCat, idCat FROM categorie JOIN user_cat ON user_cat.refCat = categorie.idCat AND user_cat.refUser = ?');
...
function addCategorieToMember($idMember, $idCat) {
    ...
    $rqt->execute(array($idMember, $idCat));
    return getAllCategories();
}
