...
document.getElementById('publier').onclick = event => {
	...
	// POST du projet 
	// On cherche si le numéo étudiant est déjà utilisé, s'il lest

	fetch('../Profils/router.php/profil/' + form2.numEtud, { method: 'GET' })
		.then(response => {
		...
		.then(data => {
			...
			if (!data) 
			{
				fetch('../Profils/router.php/profils', { method: 'POST', body: JSON.stringify(form2)})
					.then(response => {
						console.log('response 2 profils : ', response)
						return response.json()
					})
					.then(dataProfil => {
						...
						fetch('./router.php/projets', { method: 'POST', body: JSON.stringify(form) })
							...
							.then(dataProjet => {
								// maj de la liste des projets 
								console.log('dataProjet : ', dataProjet);
								displayProjet(dataProjet);
...
function voirProjet(id) {
	...
	fetch("./router.php/projet/" + id, { method: 'GET' })
		.then(response => 
			{	console.log("first rep", response);			
				return response.json()
			})
		.then(data => {
...
function recupForm(string) {
	...
	if (string == 'add') {
		form.numEtud = document.getElementById('input-numEtudiant').value;
		...
		form.presentation = document.getElementById('input-presentation').value;
	}

	/* Projet */

	if (string == 'input') {
		form.numEtud = document.getElementById(string + '-numEtudiant').value;
		form.titre = document.getElementById(string + '-titre').value;
		form.presentation = document.getElementById(string + '-presentationProjet').value;
